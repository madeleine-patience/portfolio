      ...
      <nav className={`${menuOpen ? 'block mx-auto' : 'hidden lg:block'}`}>
        ...
        >
          {menuItemTwo.map((item) => (
            <NavLink
              key={item.id}
              activeClassName=" text-white  bg-gradient-to-r from-[#aaa0eb] to-[#a299da] linked"
              inactiveClassName=" transition-all duration-300 ease-in-out dark:hover:text-white dark:bg-[#212425] hover:text-white   hover:bg-gradient-to-r from-[#aaa0eb] to-[#a299da] "
              className={`${
                ...
                a.pathname === '/homeTwo' && item.id === '01'
                  ? ' lg:text-white lg:dark:text-white   lg:bg-gradient-to-r from-[#aaa0eb] to-[#a299da] '
                  : ' '
              }`}
              to={item?.link}
            >
